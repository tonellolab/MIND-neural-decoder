...
class MIND():
    ...
    def train(self, epochs, batch_size=40, noise_model='AWGN'):
        ...
        for epoch in range(epochs):
            ...
            noise_rx = eps * np.random.normal(0, 1, (batch_size, self.latent_dim))
            noise_y = eps * np.random.normal(0, 1, (batch_size, self.latent_dim))

            # Add the Bernoulli component to get the truncated Middleton model
            if noise_model=='Middleton':
                K = 5
                noise_rx = noise_rx + np.random.binomial(size=(batch_size, self.latent_dim), n=1, p= 0.05)*(np.sqrt(K**2-1))*eps*np.random.normal(0, 1, (batch_size, self.latent_dim))
                noise_y = noise_y + np.random.binomial(size=(batch_size, self.latent_dim), n=1, p= 0.05)*(np.sqrt(K**2-1))*eps*np.random.normal(0, 1, (batch_size, self.latent_dim))

            data_rx = data_tx + noise_rx

            # Sample from the marginal of the received samples
            data_y = 2*np.random.randint(2, size=(batch_size, self.latent_dim))-1 + noise_y
    ...
    def test(self, test_size=1000, noise_model='AWGN'):
        eps = self.eps
        ...
        if noise_model == 'Middleton':
            K = 5
            bernoulli_event = np.random.binomial(size=(test_size, self.latent_dim), n=1, p=0.05)
            bernoulli_noise = bernoulli_event*(np.sqrt(K**2-1))*eps*np.random.normal(0, 1, (test_size, self.latent_dim))
        ...
        for i in range(test_size):
            ...
            # Genie decoder
            if noise_model == 'Middleton':
                variance_rx = eps**2 + bernoulli_event[i,:]*(K**2 - 1)*(eps**2) # needed for the genie Middleton decoder
                max_idx_genie = get_max_idx_loglikelihood_mid(np.expand_dims(data_rx[i,:],axis=0), training_samples, variance_rx) # map genie criterion for Middleton
            else:
                max_idx_genie = get_max_idx_loglikelihood(np.expand_dims(data_rx[i, :], axis=0),training_samples)  # maxL criterion for AWGN
            ...
            BER_maxL[0, i] = 1 - sum(logical_bits_genie) / self.latent_dim # instantaneous bit-error-rate with maxL/genie
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', help='Number of data samples to train on at once', default=32)
    ...
    parser.add_argument('--noise', help='Noise model', default='AWGN')

    args = parser.parse_args()

    test_size = int(args.test_size)
    noise_model = str(args.noise)
    ...
    ber_total_genie = np.zeros((len(SNR_dB),1))
    ...
    for SNR in SNR_dB:
        ...
        mind = MIND(k, SNR)
        # Train
        mind.train(epochs=int(args.epochs), batch_size=int(args.batch_size), noise_model=noise_model)
        # Test
        ber, ber_maxL, H_x, H_y_x, MI,P_e,y = mind.test(test_size=test_size, noise_model=noise_model)
        ber_total[j,0] = ber
        ber_total_genie[j,0] = ber_maxL
        H_x_total[j,0] = H_x
    ...
    plt.semilogy(SNR_dB, ber_total, label='MIND')
    plt.semilogy(SNR_dB, ber_total_genie, label='Genie')
    plt.semilogy(SNR_dB, P_error, label='P_error') # it makes sense to compare with SER
    plt.xlabel("Eb/N0")
    ...
    sio.savemat('MAP_MIND.mat', {'SNR': SNR_dB, 'BER': ber_total, 'BER_genie': ber_total_genie, 'source_entropy': H_x_total,'cond_entropy': H_y_x_total,'MI': MI_total, 'P_error': P_error,'data_rx': data_rx})
